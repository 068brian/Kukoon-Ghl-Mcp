...
export const rawTools = [
...

  {
    ...
    description:
      "Search opportunities, optionally filtered by pipeline, stage, status, or assigned user. Use list_pipelines first to get valid pipelineId/stageId values, and list_staff for assignedTo IDs.",
    inputSchema: z.object({
  ...
  {
    name: "get_contact",
    description: "Get full detail for a single contact by contact ID, including custom field values.",
    inputSchema: z.object({
      contactId: z.string(),
    }),
    handler: async (input: any) => {
...

  {
    ...
    description:
      "Search contacts by name, email, phone, or general query text. Returns basic contact info; use get_contact for full detail on a specific one.",
    inputSchema: z.object({
...

  {
    ...
    description:
      "List calendar/appointment events for Kukoon within an optional date range (ISO 8601 dates).",
    inputSchema: z.object({
      startDate: z.string().optional().describe("ISO date, e.g. 2026-08-01"),
      endDate: z.string().optional().describe("ISO date, e.g. 2026-08-31"),
    }),
    handler: async (input: any) => {
